...
class Faceformer(nn.Module):
    ...
    def forward(self, audio, template, vertice, one_hot, criterion,teacher_forcing=True):
        ...
        if vertice.shape[1] > self.max_frame_allowed:
            end_range = vertice.shape[1] - (self.max_frame_allowed + 1)
            start_frame = random.randint(0, end_range)
            end_frame = start_frame + self.max_frame_allowed
            vertice = vertice[:, start_frame:end_frame, :]
            hidden_states = hidden_states[:, 2 * start_frame:2 * end_frame, :]
        frame_num = vertice.shape[1]

        # print("vertice shape: ", vertice.shape)
    ...
    def predict(self, audio, template, one_hot):
        ...
        elif self.dataset == "multiface":
            frame_num = hidden_states.shape[1]
        elif self.dataset == "multiface_2":
            frame_num = hidden_states.shape[1] 
        hidden_states = self.audio_feature_map(hidden_states)
